    ...
    : IDictionary<string, object>
{
    ...
    public void Clear()
    {
        foreach (var entry in _entries) entry.Clear();
    }

    bool ICollection<KeyValuePair<string, object>>.Contains(KeyValuePair<string, object> item)
    {
        return TryGetValue(item.Key, out var value) && Equals(item.Value, value);
    }
    ...
    {
        if (!((ICollection<KeyValuePair<string, object>>)this).Contains(item)) return false;
    ...
    public sealed class Builder
    {
        ...
        public Builder Add<TValue>(string prefix, IDictionary<string, TValue> dict,
            Func<object, TValue> valueConverter = null)
        {
            ArgumentNullException.ThrowIfNull(prefix);
            ArgumentNullException.ThrowIfNull(dict);

            valueConverter ??= o => (TValue)o;
            if (_entries.Exists(e => e.Prefix == prefix)) throw new InvalidOperationException();
            _entries.Add(new Entry
            {
                ...
                TryGetValue =
                    key =>
                    {
                        var getted = dict.TryGetValue(key, out var value);
        ...
        private static IEnumerable<KeyValuePair<string, object>> Enumerate<TValue>(
            IEnumerable<KeyValuePair<string, TValue>> source)
        {
            foreach (var item in source) yield return new KeyValuePair<string, object>(item.Key, item.Value);
        }
